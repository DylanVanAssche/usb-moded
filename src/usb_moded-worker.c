...
static bool        worker_stop_daemon              (void);
static bool        worker_start_daemon             (void);
...
worker_stop_daemon(void)
{
    ...
    int rc = common_system(data->ffs_daemon_stop);
    if( rc != 0 ) {
        log_warning("failed to stop FunctionFS daemon; exit code = %d", rc);
        goto FAILURE;
    }
    ...
    log_debug("FunctionFS daemon has stopped");
...
worker_start_daemon(void)
{
    ...
    int rc = common_system(data->ffs_daemon_start);
    if( rc != 0 ) {
        log_warning("failed to start FunctionFS daemon; exit code = %d", rc);
        goto FAILURE;
    }
    ...
    log_debug("FunctionFS daemon has started");
